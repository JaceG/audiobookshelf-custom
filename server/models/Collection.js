...
class Collection extends Model {
   ...
   */
  static async getOldCollectionsJsonExpanded(user, libraryId, include, onlyUserItems = false) {
    ...
    if (onlyUserItems) {
      const userCollections = await Database.userCollection.findAll({ where: { userId: user.id } })
      const userCollectionsIds = userCollections.map((userCollection) => {
        return userCollection.collectionId
      })
      if (!collectionWhere) collectionWhere = {}
      collectionWhere.id = {
        [Sequelize.Op.in]: userCollectionsIds || []
      }
    }
