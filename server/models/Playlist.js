...
class Playlist extends Model {
   ...
   */
  static async getOldPlaylistsForUserAndLibrary(userId, libraryId = null, onlyUserItems = false) {
    ...
    if (onlyUserItems) {
      const userPlaylists = await Database.userPlaylist.findAll({ where: { userId } })
      const userPlaylistIds = userPlaylists.map((userPlaylist) => {
        return userPlaylist.playlistId
      })
      whereQuery.id = {
        [Op.in]: userPlaylistIds || []
      }
    }
