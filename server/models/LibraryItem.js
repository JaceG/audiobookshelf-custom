...
class LibraryItem extends Model {
   ...
   */
  static async getByFilterAndSort(library, user, options, onlyUserItems = false) {
    let start = Date.now()
    ...
    const userLibraryItems = await Database.userLibraryItemModel.findAll({ where: { userId: user.id } })
    console.log({ userLibraryItems })
    const items = libraryItems
      .filter((li) => {
        if (onlyUserItems) {
          return userLibraryItems.find((userLibItem) => userLibItem.libId === li.id)
        } else {
          return true
        }
      })
   ...
   */
  static async getPersonalizedShelves(library, user, include, limit, onlyUserItems = false) {
    console.log({ onlyUserItems })
    ...
    if (library.isBook) {
      ...
      const continueSeriesPayload = await libraryFilters.getLibraryItemsContinueSeries(library, user, include, limit, onlyUserItems)
    ...
    const mostRecentPayload = await libraryFilters.getLibraryItemsMostRecentlyAdded(library, user, include, limit, onlyUserItems)
    ...
    if (library.isBook) {
      start = Date.now()
      // "Recent Series" shelf
      const seriesMostRecentPayload = await libraryFilters.getSeriesMostRecentlyAdded(library, user, include, 5, onlyUserItems)
      ...
      start = Date.now()
      // "Discover" shelf
      const discoverLibraryItemsPayload = await libraryFilters.getLibraryItemsToDiscover(library, user, include, limit, onlyUserItems)
    ...
    if (library.isBook) {
      ...
      const newestAuthorsPayload = await libraryFilters.getNewestAuthors(library, user, limit, onlyUserItems)
