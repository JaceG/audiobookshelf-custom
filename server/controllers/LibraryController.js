...
class LibraryController {
  ...
  async getLibraryItems(req, res) {
    ...
    if (filterByGroup === 'series' && filterByValue !== 'no-series' && payload.collapseseries) {
      ...
      const { libraryItems, count } = await Database.libraryItemModel.getByFilterAndSort(req.library, req.user, payload, Boolean(Number(personalProfile)))
  ...
  async getAllSeriesForLibrary(req, res) {
    ...
    const { series, count } = await seriesFilters.getFilteredSeries(req.library, req.user, payload.filterBy, payload.sortBy, payload.sortDesc, include, payload.limit, offset, Boolean(Number(personalProfile)))
  ...
  async getCollectionsForLibrary(req, res) {
    ...
    let collections = await Database.collectionModel.getOldCollectionsJsonExpanded(req.user, req.library.id, include, Boolean(Number(personalProfile)))
  ...
  async getUserPlaylistsForLibrary(req, res) {
    const personalProfile = req.query.personalProfile || 0
    let playlistsForUser = await Database.playlistModel.getOldPlaylistsForUserAndLibrary(req.user.id, req.library.id, Boolean(Number(personalProfile)))
  ...
  async getUserPersonalizedShelves(req, res) {
    ...
    const shelves = await Database.libraryItemModel.getPersonalizedShelves(req.library, req.user, include, limitPerShelf, Boolean(Number(personalProfile)))
    res.json(shelves)
  }
  ...
  async search(req, res) {
    ...
    const matches = await libraryItemFilters.search(req.user, req.library, query, limit, Boolean(Number(personalProfile)))
    res.json(matches)
  }
  ...
  async getAuthors(req, res) {
    ...
    const personalProfile = Boolean(Number(req.query.personalProfile || 0))
