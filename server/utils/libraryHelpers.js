...
module.exports = {
   ...
   * @returns {Object[]}
   */
  async handleCollapseSubseries(payload, seriesId, user, library, onlyUserItems = false) {
    ...
    let libraryItems = books
      ...
      .filter((li) => {
        if (onlyUserItems) {
          return user.checkCanAccessLibraryItem(li) && userLibraryItems.find((userLibItem) => userLibItem.libId === li.id)
        }
