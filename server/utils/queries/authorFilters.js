...
module.exports = {
   ...
   */
  async search(libraryId, query, limit, offset, userBookIds = [], onlyUserItems = false) {
    ...
    let authors = await Database.authorModel.findAll({
      where: authorWhereCondition,
      attributes: {
    ...
    authors = authors.filter((author) => {
      if (onlyUserItems) {
        return Boolean(author.books.find((book) => userBookIds.includes(book.id)))
      }
      return true
    })
