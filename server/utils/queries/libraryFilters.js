...
module.exports = {
   ...
   */
  async getMediaItemsInProgress(library, user, include, limit, onlyUserItems = false) {
    if (library.isBook) {
      const { libraryItems, count } = await libraryItemsBookFilters.getFilteredLibraryItems(library.id, user, 'progress', 'in-progress', 'progress', true, false, include, limit, 0, true, onlyUserItems)
      return {
   ...
   * @returns {object} { libraryItems:LibraryItem[], count:number }
   */
  async getLibraryItemsMostRecentlyAdded(library, user, include, limit, onlyUserItems = false) {
    if (library.isBook) {
      const { libraryItems, count } = await libraryItemsBookFilters.getFilteredLibraryItems(library.id, user, 'recent', null, 'addedAt', true, false, include, limit, 0, false, onlyUserItems)
      return {
   ...
   * @returns {object} { libraryItems:LibraryItem[], count:number }
   */
  async getLibraryItemsContinueSeries(library, user, include, limit, onlyUserItems = false) {
    const { libraryItems, count } = await libraryItemsBookFilters.getContinueSeriesLibraryItems(library, user, include, limit, 0, onlyUserItems)
    return {
      libraryItems: libraryItems.map((li) => {
        const oldLibraryItem = Database.libraryItemModel.getOldLibraryItem(li).toJSONMinified()
        if (li.rssFeed) {
   ...
   */
  async getSeriesMostRecentlyAdded(library, user, include, limit, onlyUserItems = false) {
   ...
   */
  async getNewestAuthors(library, user, limit, onlyUserItems = false) {
   ...
   */
  async getLibraryItemsToDiscover(library, user, include, limit, onlyUserItems = false) {
    if (library.mediaType !== 'book') return { libraryItems: [], count: 0 }

    const { libraryItems, count } = await libraryItemsBookFilters.getDiscoverLibraryItems(library.id, user, include, limit, onlyUserItems)
    return {
      libraryItems: libraryItems.map((li) => {
        const oldLibraryItem = Database.libraryItemModel.getOldLibraryItem(li).toJSONMinified()
        if (li.rssFeed) {
