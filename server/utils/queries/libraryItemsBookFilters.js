...
module.exports = {
   ...
   */
  async getFilteredLibraryItems(libraryId, user, filterGroup, filterValue, sortBy, sortDesc, collapseseries, include, limit, offset, isHomePage = false, onlyUserItems = false) {
   ...
   */
  async getContinueSeriesLibraryItems(library, user, include, limit, offset, onlyUserItems = false) {
    ...
    let { rows: series, count } = await Database.seriesModel.findAndCountAll({
      where: [
        {
          id: {
    ...
    if (onlyUserItems) {
      series = series.filter((s) => Boolean(s.bookSeries.find((bs) => userBookIds.includes(bs.bookId))))
      count = series.length
    }
   ...
   * @returns {object} {libraryItems:LibraryItem, count:number}
   */
  async getDiscoverLibraryItems(libraryId, user, include, limit, onlyUserItems = false) {
    const userPermissionBookWhere = this.getUserPermissionBookWhereQuery(user)
   ...
   */
  async search(user, library, query, limit, offset, onlyUserItems = false) {
    const userPermissionBookWhere = this.getUserPermissionBookWhereQuery(user)
    ...
    let narratorQuery = `SELECT value, count(*) AS numBooks FROM books b, libraryItems li, json_each(b.narrators) WHERE json_valid(b.narrators) AND ${matchJsonValue} AND b.id = li.mediaId AND li.libraryId = :libraryId GROUP BY value LIMIT :limit OFFSET :offset;`
    if (onlyUserItems) {
      narratorQuery = `SELECT value, count(*) AS numBooks FROM books b, libraryItems li, json_each(b.narrators) WHERE json_valid(b.narrators) AND ${matchJsonValue} AND b.id = li.mediaId AND li.libraryId = :libraryId AND b.id IN (${userBookIds.map((id) => `"${id}"`).join(',')}) GROUP BY value LIMIT :limit OFFSET :offset;`
    }
    const [narratorResults] = await Database.sequelize.query(narratorQuery, {
      replacements: {
        libraryId: library.id,
        limit,
        offset
    ...
    // Search tags
    const tagMatches = []
    let tagQuery = `SELECT value, count(*) AS numItems FROM books b, libraryItems li, json_each(b.tags) WHERE json_valid(b.tags) AND ${matchJsonValue} AND b.id = li.mediaId AND li.libraryId = :libraryId GROUP BY value ORDER BY numItems DESC LIMIT :limit OFFSET :offset;`
    if (onlyUserItems) {
      tagQuery = `SELECT value, count(*) AS numItems FROM books b, libraryItems li, json_each(b.tags) WHERE json_valid(b.tags) AND ${matchJsonValue} AND b.id = li.mediaId AND li.libraryId = :libraryId AND b.id IN (${userBookIds.map((id) => `"${id}"`).join(',')}) GROUP BY value ORDER BY numItems DESC LIMIT :limit OFFSET :offset;`
    }
    const [tagResults] = await Database.sequelize.query(tagQuery, {
      replacements: {
        libraryId: library.id,
        limit,
        offset
    ...
    // Search genres
    const genreMatches = []
    let genreQuery = `SELECT value, count(*) AS numItems FROM books b, libraryItems li, json_each(b.genres) WHERE json_valid(b.genres) AND ${matchJsonValue} AND b.id = li.mediaId AND li.libraryId = :libraryId GROUP BY value ORDER BY numItems DESC LIMIT :limit OFFSET :offset;`
    if (onlyUserItems) {
      genreQuery = `SELECT value, count(*) AS numItems FROM books b, libraryItems li, json_each(b.tags) WHERE json_valid(b.tags) AND ${matchJsonValue} AND b.id = li.mediaId AND li.libraryId = :libraryId AND b.id IN (${userBookIds.map((id) => `"${id}"`).join(',')}) GROUP BY value ORDER BY numItems DESC LIMIT :limit OFFSET :offset;`
    }
    const [genreResults] = await Database.sequelize.query(genreQuery, {
      replacements: {
        libraryId: library.id,
        limit,
        offset
    ...
    // Search authors
    const authorMatches = await authorFilters.search(library.id, textSearchQuery, limit, offset, userBookIds, onlyUserItems)
